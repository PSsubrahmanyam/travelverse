      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8 max-w-5xl mx-auto bg-slate-950/80 p-2 rounded-2xl border border-slate-800 shadow-xl backdrop-blur-md">
          ...
          >
            ...
            <span>Live Spot Train Tracker</span>
          </button>
                ...
                searchResults.map((train) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end space-x-2 pt-2 border-t border-slate-900">
                      ...
                      <button
                        onClick={() => {
                          setLiveTrainQuery(train.number);
                          setLiveTrainData(getLiveTrainStatus(train.number));
                          setActiveSubTab('live');
                        }}
                        className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs px-4 py-2 rounded-xl transition-all shadow-md flex items-center space-x-1.5 cursor-pointer"
                      >
                        <Train className="w-3.5 h-3.5" />
                        <span>Spot Live Train</span>
                      </button>
                    </div>
        ...
        {/* ========================================================================= */}
        {/* TAB 2: LIVE SPOT TRAIN TRACKER                                             */}
        {/* ========================================================================= */}
        {activeSubTab === 'live' && (
          <div className="space-y-8 animate-fadeIn">
            
            {/* Train Query Input Box */}
            <div className="bg-slate-950 border border-slate-800 rounded-3xl p-6 shadow-2xl max-w-3xl mx-auto space-y-4">
              <form onSubmit={handleSearchLiveTrain} className="flex gap-2">
                <div className="relative flex-1">
                  <input
                    type="text"
                    value={liveTrainQuery}
                    onChange={(e) => setLiveTrainQuery(e.target.value)}
                    placeholder="Enter Train No. or Name (e.g. 12727, 20833, Godavari Express...)"
              ...
              {/* Signature Where Is My Train Inside / Outside Cell Tower GPS Bar */}
              <div className="flex items-center justify-between bg-slate-900 p-2.5 rounded-2xl border border-slate-800 text-xs">
                <span className="text-slate-400 font-bold">Location Source:</span>
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => setIsInsideTrainMode(true)}
                    className={`px-3 py-1 rounded-xl font-bold flex items-center space-x-1 transition-all ${
                      isInsideTrainMode
                  ...
                  >
                    ...
                    <span>Inside Train (Cell Tower / GPS)</span>
                  </button>

                  <button
                    onClick={() => setIsInsideTrainMode(false)}
                    className={`px-3 py-1 rounded-xl font-bold flex items-center space-x-1 transition-all ${
                      !isInsideTrainMode
                  ...
                  >
                    ...
                    <span>Outside Train (Internet)</span>
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2 text-xs overflow-x-auto">
                <span className="text-slate-500 font-bold text-[10px]">Quick Spot:</span>
                {['12727', '20833', '12839', '12760', '12805', '12951'].map((num) => (
                  <button
                    key={num}
                    onClick={() => {
                      setLiveTrainQuery(num);
                      setLiveTrainData(getLiveTrainStatus(num));
                    }}
            ...
            {/* Live Spotting Display */}
            {liveTrainData && (
              <div className="max-w-4xl mx-auto space-y-6">
                
                {/* Current Location Banner Card */}
                <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Next Stop: <strong className="text-amber-400">{liveTrainData.nextStopName} ({liveTrainData.nextStopCode})</strong> • ETA: {liveTrainData.nextStopEta}
                      </p>
                    </div>

                    {/* Live Speedometer & Alarm Toggle */}
                    <div className="flex flex-col items-end space-y-2">
                      <div className="bg-slate-900 border border-slate-800 p-2 rounded-2xl flex items-center space-x-2">
                        ...
                        <span className="text-xs font-mono font-bold text-white">
                          Speed: <strong className="text-amber-400">{liveTrainData.speed}</strong>
                        </span>
                      </div>

                      <button
                        onClick={() => setAlarmActive(!alarmActive)}
                        className={`text-xs font-bold px-3 py-1.5 rounded-full border transition-all flex items-center space-x-1 cursor-pointer ${
                          alarmActive
                            ? 'bg-amber-500 text-slate-950 border-amber-400 shadow-lg shadow-amber-500/20'
                            : 'bg-slate-900 text-slate-400 border-slate-800 hover:text-white'
                        }`}
                      >
                        {alarmActive ? <BellRing className="w-3.5 h-3.5 text-slate-950 animate-bounce" /> : <Bell className="w-3.5 h-3.5" />}
                        <span>{alarmActive ? 'Alarm Set for Destination' : 'Set Arrival Alarm'}</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-800">
                    <div className="flex justify-between text-xs text-slate-400 mb-1 font-mono">
                      <span>Distance Covered: {liveTrainData.distanceCoveredKm} km</span>
                      <span>Total: {liveTrainData.totalDistanceKm} km</span>
                    </div>
                    <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden">
                      <div
                        className="bg-gradient-to-r from-amber-500 to-emerald-400 h-full rounded-full transition-all duration-1000"
                        style={{
                          width: `${Math.min(100, Math.max(10, (liveTrainData.distanceCoveredKm / liveTrainData.totalDistanceKm) * 100))}%`
                        }}
                      />
                    </div>
                  </div>
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-3xl p-6 shadow-2xl">
                  <h4 className="font-extrabold text-white text-base mb-6 flex items-center">
                    <Layers className="w-5 h-5 text-amber-400 mr-2" />
                    Full Route Schedule & Live Station Timeline
                  </h4>

                  <div className="space-y-6 relative before:absolute before:left-6 before:top-3 before:bottom-3 before:w-0.5 before:bg-slate-800">
                    {liveTrainData.schedule.map((stop) => (
                      <div key={stop.stationCode} className="relative flex items-start space-x-6 z-10">
                        
                        {/* Station Status Pin */}
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-xs shadow-md border ${
                          stop.isCurrent
                            ? 'bg-emerald-500 text-slate-950 border-emerald-300 animate-pulse scale-110'
                            : stop.isPassed
                        ...
                        {/* Station Details */}
                        <div className="flex-1 bg-slate-900/60 p-4 rounded-2xl border border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                          <div>
                            <div className="flex items-center space-x-2">
                            ...
                            <span className="text-[11px] text-slate-400 block mt-0.5">
                              Distance: {stop.distanceKm} km • Platform #{stop.pf}
                            </span>
                          </div>
